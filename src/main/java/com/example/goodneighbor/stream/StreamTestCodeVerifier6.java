package com.example.goodneighbor.stream;

import java.util.List;

public class StreamTestCodeVerifier6 {
	...
	public static List<AnimeNameAndCountry> problem7() {
		return StreamTestDataSupplier.getAnimeList().stream().filter(it -> it.revenue >= 500)
				.map(it -> new AnimeNameAndCountry(it.name, it.country)).toList();
	}

	// 8번문제 : 매출액이 300이상인 애니메이션을 제조한 국가들의 목록을 주세요. 중복없이.

	public static List<String> problem8() {
		return StreamTestDataSupplier.getAnimeList().stream().filter(it -> it.revenue >= 300).map(it -> it.country)
				.distinct().toList();
